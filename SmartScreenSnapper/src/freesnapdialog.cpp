...
void FreeSnapDialog::mouseMoveEvent(QMouseEvent *event)
{
    ...
    //注意要向上取整ceil两次
    int x = ceil(event->x() - scaledPreviewWidth / 2);
    int y = ceil(event->y() - scaledPreviewHeight / 2);
    QPixmap noScaledPreviewPixmap = picture.copy(x, y, scaledPreviewWidth, scaledPreviewHeight);
    ...
    int drawX = std::max(-x, 0);
    int drawY = std::max(-y, 0);
    // 绘制黑色填充背景
    painter.fillRect(0, 0, scaledPreviewWidth, scaledPreviewHeight, Qt::black);
    // 绘制noScaledPreviewPixmap
    painter.drawPixmap(drawX, drawY, noScaledPreviewPixmap);
